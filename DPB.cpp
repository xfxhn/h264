...
void DPB::Modification_process_for_reference_picture_lists(const SliceHeader& sHeader)
{
	...
	if ((SLIECETYPE)sHeader.slice_type == SLIECETYPE::H264_SLIECE_TYPE_B && sHeader.ref_pic_list_modification_flag_l1)
	{
		int refIdxL1 = 0;
		int picNumL1Pred = sHeader.CurrPicNum;

		for (size_t i = 0; i < sHeader.ref_pic_list_modification_count_l0; i++)
		{
			if (sHeader.modification_of_pic_nums_idc[1][i] == 0 || sHeader.modification_of_pic_nums_idc[1][i] == 1)
			{
				//短期参考帧的重排序
				Modification_process_of_reference_picture_lists_for_short_term_reference_pictures(refIdxL1, picNumL1Pred,
					sHeader.abs_diff_pic_num_minus1[1][i], sHeader.modification_of_pic_nums_idc[1][i], sHeader.num_ref_idx_l1_active_minus1, RefPicList0, sHeader);
			}
			else if (sHeader.modification_of_pic_nums_idc[1][i] == 2)
			{
				////长期参考帧的重排序
				Modification_process_of_reference_picture_lists_for_long_term_reference_pictures(refIdxL1, picNumL1Pred,
					sHeader.long_term_pic_num[1][i], sHeader.num_ref_idx_l1_active_minus1, RefPicList1, sHeader);
			}
			else // ==3
			{
				break;
